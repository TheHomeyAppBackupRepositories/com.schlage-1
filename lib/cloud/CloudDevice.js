...
class CloudDevice extends homey_oauth2app_1.OAuth2Device {
    ...
    async onOAuth2Init() {
        ...
        if (deviceStatus) {
            ...
            // Set battery state
            await this.setBatteryValue(deviceStatus.batteryState).catch(this.error);
            // Set available
        ...
        this.registerCapabilityListener('locked', async (capabilityValue) => {
            ...
            if (capabilityValue) {
                ...
                await app.triggerLock(this, { source: 'Homey' }).catch(this.error);
            }
            else {
                await this.oAuth2Client.unlock(this.id).catch(this.error);
                await app.triggerUnlock(this, { source: 'Homey' }).catch(this.error);
            }
        });
